...
function addIframeDiv(tempArray){
	...
	for(var i=0;i<tempArray.length;i++){
			...
			if((tempArray[i][5] != undefined)&&(divType == 'mask')&&($('div#mask').size() == 0)){
				divMaskContent = '<div id="mask"><div id="mask_close"><a href="#" onclick="closeFrame();return false;">[X]</a></div></div>';
...
function getDataToDiv(Num, divType, tempArray){
	...
	$.get( tempArray[Num][0], function( data ) {
				...
				if((divType != undefined)&&(divType == 'mask')){
					divContent = '<div id="iframe_'+Num+'" class="layer_mask" style="border:none; overflow:hidden; position:fixed; left:10px; top:30px; width:'+divWidth+'px; height:'+divHeight+'px;"></div>';
				}else{
...
function addPopupMaskButton(tempBtnArray){
	...
	scriptCallFrame = "<style>#mask {position: fixed;top: 0px;left: 0px;width: 100%;height: 100%;background-color: #000;opacity: 0.8;z-index: 200;display: none;}";
	scriptCallFrame += "#mask_close {position: fixed;top: 10px;left: 10px;font-size: 15px;font-weight: bold;color: #fff;z-index: 201;}";
	scriptCallFrame += "#mask_close a {	color: #fff;text-decoration: none;}";
	scriptCallFrame += "div.layer_mask {display: none;background-color: #fff;z-index: 202;padding-top: 10px;}</style>";
	scriptCallFrame += "<script>function callFrame(popupDiv) {$('#mask').show();$('div.layer_mask').hide();popupDiv.show();}function closeFrame() {replay();$('div.layer_mask').hide();$('#mask').hide();}</script>";
...
function buttonBind(handleIframeNum){
	$('a#action_iframe_'+handleIframeNum).bind('click', function(){
		...
		$('div#mask_close').css('left', ($('div#iframe_'+handleIframeNum).width()-15)+'px');
