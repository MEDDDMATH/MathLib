...
function addPopupMaskButton(tempBtnArray){
	...
	scriptCallFrame += "<script>function callFrame(popupDiv) {$('#mask').show();$('#mask_close').show();$('div.layer_mask').hide();popupDiv.show();}function closeFrame() {$('div.layer_mask').hide();$('#mask').hide();$('#mask_close').hide();}</script>";
	$("body").append(scriptCallFrame);
	for(var a=0;a<tempBtnArray.length;a++){
		buttonType = tempBtnArray[a][3];
		buttonImage = '';
		if(buttonType=='auto'){
			buttonImage = 'ICON_AUTO.png';
		}else if(buttonType=='step'){
			buttonImage = 'ICON_STEP.png';
		}
		var subBtnFolder = copyData[tempBtnArray[a][0]][0].split("/");
		divMaskButton = '<a href="#" style="border:none; position:absolute; left:'+tempBtnArray[a][1]+'px; top:'+tempBtnArray[a][2]+'px; width:45px; height:55px;" id="action_iframe_'+tempBtnArray[a][0]+'"><img src="'+buttonImage+'"></a>';
