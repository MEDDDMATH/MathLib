...
function addStepGroup(tempGpArray){
	...
	function addStepButton(tempBtnArray){
		for(var j=0;j<tempBtnArray.length;j++){
			...
			if($('div#'+divGroupName+' a.'+buttonFile).size() == 0){
				buttonContent = '<a class="'+buttonFile+'" id="'+buttonContorlDiv+'" href="#" style="z-index:99;border:none; position:absolute; left:'+buttonLeft+'px; top:'+buttonTop+'px; width:'+buttonWidth+'px; height:'+buttonHeight+'px;"><img style="border:none;" src="'+buttonFileName+'"/></a>';
	...
	function addIframeStep(tempArray){
		...
		for(var k=0;k<tempArray.length;k++){
			...
			subFolderRoot[k] = '';
			for(var a=0;a<(subFolderNum[k]-1);a++){
		...
		function getIframeData(Num){
			...
			$.get( tempArray[Num][0], function( data ) {
				...
				data = data.replace(/images/g, subFolder[Num][0]+'/1/images')
				.replace(/"js/g, '"'+subFolder[Num][0]+'/js')
				.replace(re, '../common/')
				.replace(re2, subFolderRoot[Num]+'PAGE')
